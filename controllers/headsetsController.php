...
function postProduct($titulo,$feature1,$feature2,$feature3,$price,$image,$href,$redirect){
    ...
    try{
        ...
        if($query->rowCount() === 0){
            ...
            if($redirect){
                header('Location: http://localhost/Proyecto/views/headsets.php');//Aqui se le cambia la ruta a la página de productos actual
            }
            else{
                echo "Registro guardado";
            }
...
function putProduct($id,$titulo,$feature1,$feature2,$feature3,$price,$image,$href,$redirect){
    ...
    try{
        $query = $connection->prepare('UPDATE headsets SET titulo = :titulo, feature1 = :feature1, feature2 = :feature2, feature3 = :feature3, price = :price, image = :image, href = :href WHERE id = :id');//Para actualizar es con una coma
        $query->bindParam(':id', $id, PDO::PARAM_INT);
        $query->bindParam(':titulo', $titulo, PDO::PARAM_STR);
        $query->bindParam(':feature1', $feature1, PDO::PARAM_STR);
        $query->bindParam(':feature2', $feature2, PDO::PARAM_STR);
        ...
        if($query->rowCount() === 0){
            ...
            // echo "Registro guardado";
            if($redirect){
                header('Location: http://localhost/Proyecto/views/headsets.php');
            }
            else{
                echo "Registro guardado";
            }
