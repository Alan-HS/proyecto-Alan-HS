...
function postProduct($titulo,$feature1,$feature2,$feature3,$price,$image,$href,$redirect){
    ...
    try{
        $query = $connection->prepare('INSERT INTO discos VALUES(NULL, :titulo, :feature1, :feature2, :feature3, :price, :image, :href, 1)');//Se le cambia el nombre a cds, photocards, etc dependiendo
...
function putProduct($id,$titulo,$feature1,$feature2,$feature3,$price,$image,$href,$redirect){
    ...
    try{
        $query = $connection->prepare('UPDATE discos SET titulo = :titulo, feature1 = :feature1, feature2 = :feature2, feature3 = :feature3, price = :price, image = :image, href = :href WHERE id = :id');//Para actualizar es con una coma
        $query->bindParam(':id', $id, PDO::PARAM_INT);
        $query->bindParam(':titulo', $titulo, PDO::PARAM_STR);
        $query->bindParam(':feature1', $feature1, PDO::PARAM_STR);
        $query->bindParam(':feature2', $feature2, PDO::PARAM_STR);
...
function deleteProduct($id,$redirect){
    ...
    try{
        $query = $connection->prepare('UPDATE discos SET active = 0 WHERE id = :id');//Para actualizar es con una coma
        $query->bindParam(':id', $id, PDO::PARAM_INT);
        $query->execute();
