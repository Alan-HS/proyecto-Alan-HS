...
if($_SERVER["REQUEST_METHOD"] === "GET"){
    ...
    if(array_key_exists("id",$_GET)){
        ...
        try{
            ...
            $query = $connection->prepare('SELECT * FROM mouse_teclados WHERE id = :id');//Se le cambia el nombre ya sea photocard, cds, etc
        ...
        try{
            $query = $connection->prepare('SELECT * FROM mouse_teclados WHERE active = 1');//Se le cambia el nombre ya sea photocard, cds, etc
            $query->execute();
...
function postProduct($titulo,$feature1,$feature2,$feature3,$price,$image,$href,$redirect){
    ...
    try{
        $query = $connection->prepare('INSERT INTO mouse_teclados VALUES(NULL, :titulo, :feature1, :feature2, :feature3, :price, :image, :href, 1)');//Se le cambia el nombre a cds, photocards, etc dependiendo
        ...
        if($query->rowCount() === 0){
            ...
            if($redirect){
                header('Location: http://localhost/Proyecto/views/mouse_teclados.php');//Aqui se le cambia la ruta a la página de productos actual
            }
            else{
                echo "Registro guardado";
            }
...
function putProduct($id,$titulo,$feature1,$feature2,$feature3,$price,$image,$href,$redirect){
    ...
    try{
        $query = $connection->prepare('UPDATE mouse_teclados SET titulo = :titulo, feature1 = :feature1, feature2 = :feature2, feature3 = :feature3, price = :price, image = :image, href = :href WHERE id = :id');//Para actualizar es con una coma
        $query->bindParam(':id', $id, PDO::PARAM_INT);
        $query->bindParam(':titulo', $titulo, PDO::PARAM_STR);
        $query->bindParam(':feature1', $feature1, PDO::PARAM_STR);
        $query->bindParam(':feature2', $feature2, PDO::PARAM_STR);
        ...
        if($query->rowCount() === 0){
            ...
            // echo "Registro guardado";
            if($redirect){
                header('Location: http://localhost/Proyecto/views/mouse_teclados.php');
            }
            else{
                echo "Registro guardado";
            }
...
function deleteProduct($id,$redirect){
    ...
    try{
        $query = $connection->prepare('UPDATE mouse_teclados SET active = 0 WHERE id = :id');//Para actualizar es con una coma
        $query->bindParam(':id', $id, PDO::PARAM_INT);
        $query->execute();
